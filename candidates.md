...
_Last run: 2026-09-14 · corpus: 10470 steps / 218 sessions / 68 project dirs · 36 nodes · 167 flow edges · 12 motifs waiting_
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `N002` | request to unrecognized command | unrecognized commands | 29 | 909 | 43 | 48s | 19% | tool | script | new | Widen the vocabulary for the commands behind this node so they can be named, then split it into real candidates. |
| `N004` | linear ticket loops | linear | 46 | 1267 | 30 | 8s | 12% | tool | mcp | new | A bulk ticket upsert with a preview-before-write step, paired with the Linear project-management skills. |
...
| `N006` | inline python via heredoc | ad-hoc python | 34 | 836 | 40 | 18s | 10% | tool | script | new | A checked data-inspection helper (query, shape, diff) that replaces ad-hoc heredoc Python with one call. |
...
| `N001` | deferred tool schema fetch | tool schemas | 27 | 533 | 35 | 12s | 5% | doc | harness | new | Preload the hot deferred tools per project in settings so the first action is the work itself. |
| `N015` | github cli lookups | git | 10 | 249 | 18 | 17s | 14% | tool | skill | new | A PR-context summarizer that runs at session start. |
| `N021` | markdown draft then correction | correction | 3 | 58 | 16 | 213s | 26% | tool | skill | new | A document outline-and-approve step before the full write. |
...
| `N007` | search then rewrite loop | editing | 14 | 416 | 31 | 6s | 7% | tool | skill | new | A guarded batch-edit primitive: match, preview, apply, verify in one step. |
...
| `N018` | fireflies transcript loops | mcp bulk | 5 | 189 | 14 | 5s | 5% | tool | mcp | new | A batch transcript fetch, or preloading the Fireflies tools so the loop is one call. |
...
| `N031` | harvest time entry loops | mcp bulk | 1 | 128 | 3 | 3s | 9% | tool | mcp | new | A batch time-entry operation or a CSV-to-Harvest loader. |
...
|---|---|---|---|
| 1 | `N002` request to unrecognized command | 190769194 | 17% |
| 2 | `N004` linear ticket loops | 158477633 | 43% |
| 3 | `N003` git orientation and commit loop | 111693688 | 57% |
| 4 | `N011` subagent dispatch then correction | 57874844 | 67% |
| 5 | `N006` inline python via heredoc | 52883876 | 75% |
| 6 | `N010` direct in-place edit then correction | 32732962 | 80% |
| 7 | `N001` deferred tool schema fetch | 26989341 | 85% |
| 8 | `N015` github cli lookups | 21687418 | 88% |
| 9 | `N021` markdown draft then correction | 10347896 | 90% |
...
| `N006` inline python via heredoc | `N007` search then rewrite loop | 86 | 37 | 15s |
...
| `N004` linear ticket loops | `N002` request to unrecognized command | 27 | 26 | 25s |
...
| `N002` request to unrecognized command | `N004` linear ticket loops | 19 | 18 | 0s |
...
| `N018` fireflies transcript loops | `N007` search then rewrite loop | 14 | 10 | 23s |
...
| `N001` deferred tool schema fetch | `N018` fireflies transcript loops | 11 | 10 | 0s |
| `N005` skill invocation handoff | `N006` inline python via heredoc | 11 | 11 | 24s |
| `N007` search then rewrite loop | `N018` fireflies transcript loops | 11 | 9 | 12s |
...
| `N018` fireflies transcript loops | `N006` inline python via heredoc | 10 | 8 | 48s |
...
| `N031` harvest time entry loops | `N004` linear ticket loops | 9 | 9 | 7s |
...
| `N002` request to unrecognized command | `N031` harvest time entry loops | 8 | 7 | 23s |
...
| `N018` fireflies transcript loops | `N002` request to unrecognized command | 8 | 7 | 48s |
...
| `N004` linear ticket loops | `N031` harvest time entry loops | 6 | 5 | 44s |
...
| `N013` box document loops | `N018` fireflies transcript loops | 0.429 |
...
| `N004` linear ticket loops | `N018` fireflies transcript loops | 0.385 |
