...
_Last run: 2026-09-14 · corpus: 10417 steps / 216 sessions / 68 project dirs · 39 nodes · 179 flow edges · 16 motifs waiting_
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `N002` | request to unrecognized command | unrecognized commands | 28 | 900 | 43 | 48s | 19% | tool | script | new | Widen the vocabulary for the commands behind this node so they can be named, then split it into real candidates. |
...
| `N001` | deferred tool schema fetch | tool schemas | 20 | 450 | 35 | 12s | 6% | doc | harness | new | Preload the hot deferred tools per project in settings so the first action is the work itself. |
| `N015` | github cli lookups | git | 10 | 249 | 18 | 17s | 14% | tool | skill | new | A PR-context summarizer that runs at session start. |
| `N021` | markdown draft then correction | correction | 3 | 57 | 16 | 213s | 25% | unclassified | skill | new | A document outline-and-approve step before the full write. |
...
|---|---|---|---|
| 1 | `N002` request to unrecognized command | 190517596 | 17% |
...
| 7 | `N001` deferred tool schema fetch | 26205698 | 85% |
...
| `N002` request to unrecognized command | `N030` unrecognized tool then correction | 6 | 3 | 217s |
...
| `N002` request to unrecognized command | `N009` checkout then run | 0.364 |
| `N002` request to unrecognized command | `N016` run then search output | 0.364 |
| `N002` request to unrecognized command | `N022` locate then run | 0.364 |
| `N002` request to unrecognized command | `N028` search then git | 0.364 |
| `N002` request to unrecognized command | `N029` custom command then delegate | 0.364 |
| `N002` request to unrecognized command | `N037` git then delegate | 0.364 |
...
| `N002` request to unrecognized command | `N006` inline python via heredoc | 0.304 |
